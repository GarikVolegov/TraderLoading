...
function BrokerHubWidgetInner() {
  ...
  return (
    <Card className="relative overflow-hidden border-border/30 bg-card/60">
      ...
      <CardContent className="grid grid-cols-2 gap-2 px-4 pb-4 pt-0">
        <div className="col-span-2 rounded-lg border border-border/40 bg-secondary/20 px-3 py-2 text-[11px] text-muted-foreground">
          {subtitle}
        </div>
        <StatTile label="Balance" value={money(snapshot.metrics.balance)} unit={snapshot.metrics.currency} size="md" />
        <StatTile label="Equity" value={money(snapshot.metrics.equity)} unit={snapshot.metrics.currency} tone="primary" size="md" />
        <AccountEquityCurve
          history={history}
