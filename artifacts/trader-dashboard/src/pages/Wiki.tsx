...
interface WikiGraph {
  ...
  nodes: Array<{ id: number; label: string; type: string; communityId: number | null }>;
}
...
function StatusPill({ status }: { status: WikiStatus }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-md border border-border/40 bg-background/60 px-1.5 py-0.5 text-[10px] font-semibold ${meta.color}`}>
...
function SourceRow({ source, onDelete }: { source: WikiSource; onDelete: (id: number) => void }) {
  ...
  return (
    <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-lg border border-border/45 bg-card/60 px-3 py-2">
      ...
      <div className="min-w-0">
        ...
        {source.error && <p className="whitespace-normal break-words text-xs leading-relaxed text-red-300">{source.error}</p>}
      </div>
      <button
        type="button"
...
export default function Wiki() {
  ...
  const { data: sources = [] } = useQuery({
    ...
    refetchInterval: (query) =>
      (query.state.data as WikiSource[] | undefined)?.some((source) => ["queued", "processing"].includes(source.status))
        ? 2500
        : false,
  });
  ...
  const textMutation = useMutation({
    mutationFn: () => apiFetch<WikiSource>(`${API}/wiki/sources/text`, {
      method: "POST",
  ...
  const urlMutation = useMutation({
    mutationFn: () => apiFetch<WikiSource>(`${API}/wiki/sources/url`, {
      method: "POST",
  ...
  const queryMutation = useMutation({
    mutationFn: () => apiFetch<WikiAnswer>(`${API}/wiki/query`, {
      method: "POST",
  ...
  const filteredSources = useMemo(
    ...
    [kindFilter, sources, statusFilter],
  );
      ...
      <div className="mt-2 grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="space-y-4 min-w-0">
          <Card>
            <CardContent className="space-y-4 p-4">
              ...
              >
                <input {...getInputProps()} />
                {uploadMutation.isPending ? <Loader2 className="mb-3 h-8 w-8 animate-spin text-primary" /> : <Upload className="mb-3 h-8 w-8 text-primary" />}
                <p className="text-sm font-semibold">{uiText("wiki.upload.drop_title")}</p>
                <p className="mt-1 max-w-md text-xs text-muted-foreground">
                  Carica file locali da qualunque cartella o provider. Se il contenuto e testuale lo imparo; altrimenti conservo l'originale nella wiki.
                </p>
                <Button
                  type="button"
                  className="mt-4"
              ...
              <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
                <div className="space-y-2">
                  <Input placeholder="Titolo nota" value={noteTitle} onChange={(event) => setNoteTitle(event.target.value)} />
                  <textarea
                    value={noteText}
                  ...
                  />
                  <Button className="w-full" disabled={!noteText.trim() || textMutation.isPending} onClick={() => textMutation.mutate()}>
                    {textMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <FileText className="mr-2 h-4 w-4" />}
                    Aggiungi nota
                  </Button>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="rounded-lg border border-border/45 bg-secondary/20 p-3 text-xs leading-relaxed text-muted-foreground">
                    Importo il contenuto testuale della pagina, lo pulisco e lo collego al grafo della tua wiki personale.
                  </div>
                  <Button variant="outline" className="w-full" disabled={!url.trim() || urlMutation.isPending} onClick={() => urlMutation.mutate()}>
                    {urlMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Globe2 className="mr-2 h-4 w-4" />}
                    Importa URL
                  </Button>
                </div>
              </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <MessageSquareText className="h-4 w-4 text-primary" />{uiText("wiki.ask.title")}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                />
                <Button disabled={!question.trim() || queryMutation.isPending} onClick={() => queryMutation.mutate()}>
              ...
              {answers.length === 0 ? (
                <div className="rounded-lg border border-border/40 bg-secondary/20 p-6 text-center text-sm text-muted-foreground">
                  ...
                  Fai una domanda: Brain AI risponderà solo con ciò che trova nella tua wiki e mostrerà le fonti.
                </div>
              ) : (
                <div className="space-y-3">
                  {answers.map((answer, index) => (
                    <div key={index} className="rounded-lg border border-border/45 bg-card/60 p-3">
                      <p className="whitespace-pre-wrap text-sm leading-relaxed">{answer.answer}</p>
                      {answer.citations.length > 0 && (
                        <div className="mt-3 space-y-1.5">
                          {answer.citations.map((citation) => (
                            <div key={`${citation.sourceId}-${citation.title}`} className="rounded-md bg-background/70 px-2 py-1.5 text-xs text-muted-foreground">
                              <span className="font-semibold text-foreground">{citation.title}</span> · {citation.excerpt}
                            </div>
                          ))}
                        </div>
                      )}
        ...
        <div className="space-y-4 min-w-0">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                <Network className="h-4 w-4 text-primary" />{uiText("wiki.graph.title")}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div key={label} className="rounded-lg border border-border/40 bg-secondary/20 p-3">
              ...
              {(communities.length || graph?.communities.length) ? (
                <div className="space-y-1.5">
                  {(communities.length ? communities : graph!.communities).slice(0, 8).map((community) => (
                    <div key={community.id} className="flex items-center gap-2 rounded-md border border-border/35 px-2 py-1.5">
                      <GitBranch className="h-3.5 w-3.5 text-primary" />
                      <span className="min-w-0 flex-1 truncate text-xs font-semibold">{community.label}</span>
                      <span className="font-mono text-[10px] text-muted-foreground">{community.nodeCount}</span>
                    </div>
                  ))}
                </div>
              ) : null}
            ...
            <CardContent className="space-y-2">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                <label className="space-y-1 text-xs font-medium text-muted-foreground">
                  ...
                  <select
                    value={statusFilter}
                    onChange={(event) => setStatusFilter(event.target.value as "all" | WikiStatus)}
              ...
              <p className="text-xs text-muted-foreground">{uiText("wiki.sources.subtitle")}</p>
              {sources.length === 0 ? (
                <p className="rounded-lg border border-border/40 bg-secondary/20 p-4 text-sm text-muted-foreground">
                  Nessuna fonte ancora caricata. Aggiungi una nota, un URL o un file per iniziare a costruire il grafo.
                </p>
                ...
                filteredSources.map((source) => (
                  <SourceRow key={source.id} source={source} onDelete={(id) => deleteMutation.mutate(id)} />
                ))
              )}
            </CardContent>
          </Card>
