...
assert.match(app, /function PublicAuthLoadingFallback\(\)/);
assert.match(app, /if \(!isLoaded\) \{\s*return <PublicAuthLoadingFallback \/>;\s*\}/);
assert.match(app, /<Route path="\/" component=\{LandingPage\} \/>/);
assert.match(app, /header: "!hidden"/);
assert.match(app, /logoBox: "!hidden"/);
assert.match(app, /footer: "!hidden"/);
assert.match(app, /const configuredClerkProxyUrl = rawClerkProxyUrl\?\.replace/);
assert.match(app, /const clerkProxyDisabled = import\.meta\.env\.VITE_CLERK_USE_PROXY === "false"/);
assert.match(app, /configuredClerkProxyUrl && !clerkProxyDisabled/);
assert.doesNotMatch(app, /VITE_CLERK_USE_PROXY === "true"/);
