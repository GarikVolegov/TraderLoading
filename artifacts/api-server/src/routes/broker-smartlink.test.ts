...
try {
  ...
  let issuedSmartLinkToken = "";
  const smartLinkService: Mt5SmartLinkService = {
    async start({ profileId, token }) {
      startedProfileId = profileId;
      issuedSmartLinkToken = token ?? "";
      return {
        profileId,
  ...
  app.use("/api", createBrokersRouter(runtime, { companionStore, smartLinkService, enableLegacyConnectionRoutes: true, requireProAccess: async () => true }));
  ...
  assert.notEqual(issuedSmartLinkToken, "smartlink");
  assert.equal(issuedSmartLinkToken.length >= 32, true);
  ...
  const rejectedSnapshotRes = await fetch(`${base}/companion/snapshot`, {
    method: "POST",
  ...
  assert.equal(rejectedSnapshotRes.status, 400);
  ...
  const closePendingRes = await fetch(`${base}/companion/orders/pending?profileId=${encodeURIComponent(started.profile.id)}&token=${encodeURIComponent(issuedSmartLinkToken)}`);
  ...
  const pendingRes = await fetch(`${base}/companion/orders/pending?profileId=${encodeURIComponent(started.profile.id)}&token=${encodeURIComponent(issuedSmartLinkToken)}`);
  ...
  const orderResultRes = await fetch(`${base}/companion/orders/smartlink-order-1/result`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ profileId: started.profile.id, token: issuedSmartLinkToken, accepted: true, brokerOrderId: "mt5-1001" }),
  });
